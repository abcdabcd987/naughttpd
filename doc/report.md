...
这个例子固然简单，然而问题也是显而易见的。我们用个现实世界的场景来做类比，HTTP Server 类似于一个快递店，每个连接就是一个客户。对于每个客户的（取件）请求，工作人员需要去把相应的箱子找到并拿给客户。在这个最简单的模型中，快递店只有老板自己一个人，所以他又得接待客户，又得去找箱子。于是，当老板在为上一位客户找箱子的时候，后面的客户只能排队等待了。
...
想象刚才这个快递店边上就是一个巨大的劳动力市场。老板觉得自己累死累活还不能满足客户的需求，不如雇一点临时工。于是，老板专门在前台接待客户。每来一个客户，老板就向边上的人才市场招招手，雇一个临时工去仓库里找箱子。在把箱子交给客户之后，老板一手把劳务费交给临时工，一手就送走了临时工。
...
快递店老板发现每次雇临时工非常麻烦，于是在开张之前就招了一些正式的合同工，集中培训。营业时，老板在前台接受客户的请求，然后派一个伙计去仓库中取货。老板和这些伙计们建立了深厚的情谊（合同），直到最后关店才和伙计们说再见。

相比起前面的模型，线程池有好的地方也有不好的地方。好的地方自然是没有了额外的创建和销毁线程的开销。然而，当所有伙计都在仓库取货的时候，新来的客户依然只能等待。而如果招聘临时工的话，就没有这个问题。另外，在开张之初要招聘多少伙计，这也是一个需要权衡的问题。
...
close(srcfd);
for (ssize_t nwrite = 0; nwrite < stat.st_size; )
    nwrite += write(infd, srcaddr, stat.st_size - nwrite);
munmap(srcaddr, sbuf.st_size);
```

## 减少内存拷贝：`sendfile()`

注意到在使用 `mmap()` 的时候其实我们仍然可能多次调用了 `write()`。另外，因为 `mmap()` 是通过 Page Fault 机制来实现的，所以我们的程序有可能不断触发 Page Fault，在用户态和内核中来回跳转。
